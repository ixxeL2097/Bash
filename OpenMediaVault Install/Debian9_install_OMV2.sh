...
{
    echo -e "$YELLOW""[SCRIPT] : INSTALLING MULTIMEDIA PACKAGES...""$WHITE"
    /usr/bin/apt install synaptic apt-xapian-index gdebi gksu -y
    /usr/bin/apt install vlc -y
    /usr/bin/apt install gufw -y

    echo -e "$YELLOW""[SCRIPT] : INSTALLING VISUAL STUDIO CODE...""$WHITE"
    /usr/bin/curl https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor > microsoft.gpg
    /usr/bin/mv microsoft.gpg /etc/apt/trusted.gpg.d/microsoft.gpg
    sh -c 'echo "deb [arch=amd64] https://packages.microsoft.com/repos/vscode stable main" > /etc/apt/sources.list.d/vscode.list'
    /usr/bin/apt update
    /usr/bin/apt install code

    echo -e "$YELLOW""[SCRIPT] : INSTALLING PYTHON UTILITIES...""$WHITE"
    /usr/bin/apt install code python-dev 
    /usr/bin/apt install python3-pip
    /usr/bin/pip3 install --upgrade pip
    /usr/bin/pip3 install colorama
    /usr/bin/apt install python-pip
    /usr/bin/pip install --upgrade pip
    /usr/bin/pip install colorama
